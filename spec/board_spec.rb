...
describe Board do
  ...
  describe '#update' do
    it 'updates the grid matrix value' do
      expect { board.update(0, 0, 'PIECE') }.to change { board.grid[0][0] }.to('PIECE')
    end

    it 'returns the previous piece as captured value' do
  ...
  describe '#path_free?' do
    ...
    it 'returns false if the path is not free' do
  ...
  describe '#execute_move' do
    context 'when there is no capture' do
      let(:object) { { source: [6, 0], target: [6, 0] } }
    ...
    context 'when there is a capture' do
      let(:object) { { source: [6, 0], target: [1, 0] } }
  ...
  describe '#validate_move' do
    context 'when there is no piece in the source position' do
      let(:object) { { source: [3, 3], target: [3, 4] } }
    ...
    context 'when the white player tries to move a black piece' do
      let(:object) { { source: [1, 0], target: [2, 0] } }
      ...
      it 'raises an exception' do
        expect do
          board.validate_move(object, piece, player)
    ...
    context 'when the white player tries an invalid move' do
      let(:object) { { source: [6, 0], target: [4, 0] } }
  ...
  describe '#check?' do
    context 'when the white player tries to capture the black king' do
      ...
      it 'returns true if it is a valid check' do
