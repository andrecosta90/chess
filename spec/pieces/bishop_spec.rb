...
describe Bishop do
  ...
  describe '#same_diagonal?' do
    context 'when the target is on the same diagonal as the source' do
      ...
      it 'returns true' do
        expect(piece.same_diagonal?(target, source)).to be true
      end
    end
    ...
    context 'when the target is not on the same diagonal as the source' do
      ...
      it 'returns false' do
        expect(piece.same_diagonal?(target, source)).to be false
      end
    end
  end
  ...
  describe '#valid_movement?' do
    ...
    context 'when the bishop tries an invalid move' do
      it 'returns false' do
        expect(piece.valid_movement?(source, [6, 4], board)).to be false
      end
    end

    context 'when the bishop tries to leap' do
      it 'returns false' do
        expect(piece.valid_movement?(source, [2, 2], board)).to be false
      end
    end

    context 'when the bishop tries a valid move (i)' do
      it 'returns true' do
        expect(piece.valid_movement?(source, [2, 6], board)).to be true
      end
    end

    context 'when the bishop tries a valid move (ii)' do
      it 'returns true' do
        expect(piece.valid_movement?(source, [7, 1], board)).to be true
      end
    end

    context 'when the bishop tries a capture' do
      it 'returns true' do
        expect(piece.valid_movement?(source, [7, 7], board)).to be true
      end
    end

    context 'when a white bishop tries to capture a white piece' do
      it 'returns false' do
